...
try {
    if(isset ($_GET['id']) && is_numeric($_GET['id'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO incidencia_reparada (id_planta, id_incidencia, id_zona, id_modulo, fecha_arreglo, hora_arreglo, id_tecnico, tecnico_borro, fecha, historial, observacion) SELECT id_planta, id_incidencia, id_zona, id_modulo, current_date, current_time, id_tecnico, :tecnico_borro, fecha, historial, observacion FROM informe_incidencia WHERE id = :id');
        $stmt->bindParam(':id', $id); // this time, we'll use the bindParam method
        $stmt->bindParam(':tecnico_borro', $_SESSION['usuario']); // this time, we'll use the bindParam method
        $stmt->execute();
